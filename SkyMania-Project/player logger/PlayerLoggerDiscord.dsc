...
player_logger_discord_send_log:
    ...
    - foreach <[channels_to_send]> as:channel:
        ...
        - if <[log_result].length> >= <[discord_text_limit]>:
            - ~discordmessage id:skymania channel:<[channel]> attach_files:<map[<[id]>_<player.name>_<[category]>.yml=<[log_result]>]>
            - stop
        - define embed <discord_embed.with_map[<[log_format]>]>
        - define embed <[embed].with[description].as[**<[id]>** - **<player.name>** - **<[category]>** ```<[log_result]>```]>
        - ~discordmessage id:skymania channel:<[channel]> <[embed]>
        - wait 1s
...
player_logger_discord_commands_handler:
    ...
    events:
        ...
        - if <[args].size> == 0 || <[args].get[1]> == help:
            ...
            - ~discordmessage id:skymania reply:<[message]> <[embed]>
            - stop
        ...
        - if <[args].get[1]> == recoverdrops:
            ...
            - ~discordmessage id:skymania reply:<[message]> "Drops recovered. `/ex inventory open destination:<entry[drops_note].created_queue.determination.get[1]>`"
            - stop
        ...
        - if <[args].contains[uploaddc]>:
            ...
            - ~discordmessage id:skymania reply:<[message]> attach_files:<map[<[file_name]>=<[results].to_yaml>]>
            - stop
        ...
        - if <[args].contains[uploadpaste]>:
            ...
            - ~discordmessage id:skymania reply:<[message]> "Logs uploaded to the paste server. <n>Link: <[result_url]>"
            - stop
        ...
        - foreach <[results]>:
            ...
            - if <[log_size]> >= <[discord_text_limit]>:
                - ~discordmessage id:skymania reply:<[message]> attach_files:<map[<[id]>_<[player_name]>_<[category]>.yml=<[value].to_yaml>]>
                - foreach next
