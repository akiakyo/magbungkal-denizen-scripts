diablo_crossbow:
    type: item
    material: crossbow
    display name: <&gradient[from=#2D1B1C;to=#5A0606]>☠ Crossbow of Eternal Torment ☠
    enchantments:
    - infinity:1
    - multishot:1
    mechanisms:
         unbreakable: true
         custom_model_data: 10000003
         attribute_modifiers:
    ...
    lore:
    - <empty>
    -   <&gradient[from=#5A0606;to=#5A0606]>This crossbow, crafted from the bones of the damned,
    -   <&gradient[from=#5A0606;to=#5A0606]>fires bolts that linger in the air, inflicting pain upon all who are struck.
    - <empty>
    ...
    - <&gradient[from=#5A0606;to=#5A0606]>Skill Cooldown: <&gradient[from=#5A0606;to=#5A0606]>15s
    - <empty>
    - <&8><&l>▶ <&r><&gradient[from=#5A0606;to=#5A0606]>When <&gradient[from=#5A0606;to=#5A0606]>left-clicks <&gradient[from=#5A0606;to=#5A0606]>apply <&gradient[from=#5A0606;to=#5A0606]>Eternal Torment's Dash
...
diablo_crossbowEvents:
    type: world
    debug: false
    events:
      after player shoots diablo_crossbow:
      - if <player.location.in_region[spawn5]>:
        - actionbar "<&8><&l>[<&6><&l>!<&8><&l>] <&f>You can't use a skill at spawn!"
        - determine cancelled
      - if <player.has_flag[effect_cooldown_diablo_crossbow]>:
         - actionbar "<&8><&l>[<&6><&l>!<&8><&l>] <&f>Skill is still on cooldown: <player.flag_expiration[effect_cooldown_diablo_crossbow].from_now.formatted>"
      ...
      - flag player effect_cooldown_diablo_crossbow expire:15s

      after player holds item item:diablo_crossbow:
      - define player <player.name>
      - if <player.location.in_region[spawn5]>:
        - actionbar "<&8><&l>[<&6><&l>!<&8><&l>] <&f>You can't use a skill at spawn!"
        - determine cancelled
      - if <player.has_flag[diablo_crossbow_effect]>:
         - actionbar "<&8><&l>[<&6><&l>!<&8><&l>] <&f>Strength & Speed is still on cooldown: <player.flag_expiration[diablo_crossbow_effect].from_now.formatted>"
         - determine cancelled
      ...
      - flag player diablo_crossbow_effect expire:30m
      ...
      on player left clicks block with:diablo_crossbow:
      - define item <context.item>
      - if <player.location.in_region[spawn5]>:
        - actionbar "<&8><&l>[<&6><&l>!<&8><&l>] <&f>You can't use a skill at spawn!"
        - determine cancelled
      - define diablo_crossbow <item[diablo_crossbow]>
      - if <[item]> == <[diablo_crossbow]>:
         - if <player.has_flag[diablo_crossbow_blindness]>:
            - actionbar "<&8><&l>[<&6><&l>!<&8><&l>] <&f>You are tired! You can use this skill again after <player.flag_expiration[diablo_crossbow_blindness].from_now.formatted>"
            - determine cancelled
         ...
         - actionbar "<&8><&l>[<&6><&l>!<&8><&l>] <&f>You used Eternal Torment's Dash"
         - playsound <player.location> sound:ENTITY_ENDERMAN_TELEPORT volume:10
         - wait 2.5s
         - flag player diablo_crossbow_blindness expire:30s