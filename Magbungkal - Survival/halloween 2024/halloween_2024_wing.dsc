halloween_2024_wing:
    type: item
    material: paper
    display name: <&gradient[from=#D2D2D2;to=#8D8D8D]>☠ Wings of the Forsaken ☠
    # enchantments:
    # - looting:4
    mechanisms:
         unbreakable: true
...
halloween_2024_wingEvents:
    type: world
    debug: false
    events:
      after player holds item item:halloween_2024_wing:
      - define player <player.name>
      - if <player.has_flag[halloween_2024_wing_strength]>:
         - actionbar "<&8><&l>[<&6><&l>!<&8><&l>] <&f>Night Vision is still on cooldown: <player.flag_expiration[halloween_2024_wing_strength].from_now.formatted>"
         - determine cancelled
      ...
      - flag player halloween_2024_wing_strength expire:30m